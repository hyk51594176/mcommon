<template>
  <div>
    ...
    <h3>表单展示</h3>
    {{formData}}
    <m-form :model='formData' ref='mform' :size='options.size' no-warp :labelPosition="options.labelPosition" :columns='columns' :disabled='options.disabled' :labelWidth='options.labelWidth+"px"'>
...
export default {
        ...
        ...this.getOtherColumns(),
        {
          ...
          render:()=>{
            return <div style='text-align:center'>
              <el-button disabled={this.isDisabled('btn')} onClick={()=>{
  ...
  methods: {
    ...
    getSelectList(){
      return Promise.resolve(this.columns.map(obj=>({label:obj.label,value:obj.prop})))
    },
    getList({parent}){
      let arr = []
      if(parent === 'zhejiang'){
    ...
    getOtherColumns(){
      return this.formData.list.map((obj,index)=>{
        return [
          {
            label:`联动选择${index}`,
            ...
            prop:`del${index}`,
            span:6,
            render:<el-button type='danger' onClick={()=>{
